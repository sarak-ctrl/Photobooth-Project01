...
const canvas = document.getElementById('finalCanvas'),
     ...
     filterSelect = document.getElementById('editorFilterSelect'); // NEW filter dropdown 
...
// Apply filter only to the background photo
  ctx.filter = filterSelect?.value || 'none';
  ctx.drawImage(finalImage, 0, 0, WIDTH, HEIGHT); 

  // Reset filter for stickers so they remain normal
  ctx.filter = 'none';
  stickers.forEach(s => ctx.drawImage(s.img, s.x, s.y, s.width, s.height));
}
...
// --- NEW: redraw canvas when filter changes ---
if (filterSelect) { 
  filterSelect.addEventListener('change', drawCanvas);
}
});
